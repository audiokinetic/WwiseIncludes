...
struct AkRoomParams
{
	...
	/// Room reverb priority.  
	/// For a given sound path, a single room reverb is chosen for the emitter - the highest priority room along the chain from emitter to listener.  Note that if the selected aux bus is set to 'use game defined sends'
	/// in the Wwise Authoring tool, then the emitter's room will also feed (via a aux send) to the listener room reverb.
	/// If there is a tie between two rooms in a sound path, then the room closest to the listener is taken.  
	/// To minimize the total number of reverb effects that are instantiated at any given time, it is recommenced to keep the same priority for most rooms, and
	/// only increase the priority for a select number of 'important' rooms.
	AkInt32							Priority;
...
namespace AK
{
	...
	namespace SpatialAudio
	{
		...
		/// This function should be called instead of \c AK::SoundEngine::SetObjectObstructionAndOcclusion(), and is only needed for setting the obstruction of an object that is in the same room as the listener.  
		/// If the listener and the emitter are in different rooms, then this value is ignored and instead, the obstruction value is based on diffraction, as calculated during sound propagation.
